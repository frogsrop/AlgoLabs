...
{    	
    ...
    {
    	//cout<<my.lcp[i]<<' ';
    	if ((int)x.size() < max(my.suff_arr_data[i + 1], my.suff_arr_data[i]) && (int)x.size() > min(my.suff_arr_data[i], my.suff_arr_data[i + 1]) && my.lcp[i] > len) 
        {
			len = my.lcp[i];
			from = my.suff_arr_data[i];
        }
    }
    //cout<<endl;
    cout << s.substr(from, len);	
    return 0;
}